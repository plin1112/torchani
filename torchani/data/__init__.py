...
class AEVPCacheLoader(Dataset):
    ...
    Output:
        species_aevs
        properties
        modified to allow to output the selected properties
    """

    def __init__(self, disk_cache=None, selection=None):
        super(AEVPCacheLoader, self).__init__()
        self.disk_cache = disk_cache
    ...
    def __getitem__(self, index):
        chunk_dataset, output = self.dataset.batches[index]
        aev_path = os.path.join(self.disk_cache, str(index))
        with open(aev_path, 'rb') as f:
            species_aevs = pickle.load(f)
            for i, sa in enumerate(species_aevs):
